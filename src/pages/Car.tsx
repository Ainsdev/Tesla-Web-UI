...
function Car(props: TeslaCarType) {
    const firstImage = props.bgImages[0]
    return (
            ...
            <section className="snap-always snap-start w-screen h-max p-40 bg-black flex justify-center items-center">
                <div className="grid sm:grid-cols-2 gap-y-5 grid-rows-3 text-white text-center">
                    <article className="w-[60vh] h-max order-1">
                        <motion.div transition={{ type: 'spring', duration: 1, bounce: 0.4 }} initial={{ x: '-100vw' }} animate={{ x: 0 }}><img src={props.data[0].photo} /></motion.div>
                    </article>
                    <motion.article transition={{ type: 'spring', duration: 1, bounce: 0.4 }} initial={{ x: '100vw' }} animate={{ x: 0 }} className="w-[60vh] h-max p-5 order-2">
                        <h1 className="text-xl font-bold pb-12">{props.data[0].title}</h1>
                        <p className="text-lg px-20">{props.data[0].text}</p>
                    </motion.article>
                    <motion.article transition={{ type: 'spring', duration: 1, bounce: 0.4 }} initial={{ x: '100vw' }} animate={{ x: 0 }} className="w-[60vh] h-max order-4 sm:order-3">
                    ...
                    <motion.article className="w-[60vh] h-max p-5 order-3 sm:order-4">
                        <motion.div transition={{ type: 'spring', duration: 1, bounce: 0.4 }} initial={{ x: '-100vw' }} animate={{ x: 0 }} ><img src={props.data[1].photo} /></motion.div>
                    </motion.article>
                    <motion.article className="w-[60vh] h-max order-4">
                        <motion.div transition={{ type: 'spring', duration: 1, bounce: 0.4 }} initial={{ x: '100vw' }} animate={{ x: 0 }}><img src={props.data[2].photo} /></motion.div>
                    </motion.article>
                    <motion.article transition={{ type: 'spring', duration: 1, bounce: 0.4 }} initial={{ x: '100vw' }} animate={{ x: 0 }} className="w-[60vh] h-max p-5 order-5">
